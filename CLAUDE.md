...
- **Status visuals**: `renderSummary()` draws the header "pulse" (headline, legend, big status bar) and the Workstreams list (one bar per project; clicking a row toggles `filters.project`) from all tasks, ignoring filters. `renderStatusBar()` sizes one segment per status, with the overdue share hatched. Status colours are `--st-*` tokens, applied through `[data-status]` selectors on columns, cards, bar segments and legend items. Priority uses a neutral signal meter so that it never competes with the status colours.
- **Welcome dialog**: `wireWelcomeDialog()` opens the native `<dialog id="welcomeDialog">` once per page load after `WELCOME_DELAY_MS` (10s), and waits if the tab is in the background. The IT support hotline number is in that dialog's markup.
